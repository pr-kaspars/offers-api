...
public class OfferResource {
  ...
  public List<ListingResponse> active() {
    return service.active().stream().map(ListingResponse::new).collect(Collectors.toList());
  }

  @PostMapping("/offers")
  @ResponseStatus(HttpStatus.CREATED)
  public ListingResponse create(@Valid @RequestBody CreateOfferPayload payload, HttpServletResponse response, UriComponentsBuilder uriComponentsBuilder) {
    ...
    return new ListingResponse(offer);
  }

  @GetMapping("/offers/{id}")
  public Optional<ListingResponse> read(@PathVariable Integer id) {
    return service.read(id).map(ListingResponse::new);
  }
